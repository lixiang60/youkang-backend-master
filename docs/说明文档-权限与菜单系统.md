...
数据权限是**行级安全**机制，控制不同用户能看到**哪些行数据**。与功能权限（控制能不能访问某个接口）不同，数据权限控制的是同一个接口返回的数据范围。

例如：所有销售都能访问"订单列表"接口，但每个销售只能看到自己部门的订单，而销售总监能看到所有部门的订单——这就是数据权限的作用。

**核心思路**：通过 `@DataScope` 注解 + AOP 切面，在 SQL 执行前自动注入 `WHERE` 条件，无需业务代码手动判断。
...
| 文件 | 位置 | 职责 |
|------|------|------|
| `@DataScope` | `youkang-common/annotation/DataScope.java` | 注解定义，声明表别名 |
| `DataScopeAspect` | `youkang-framework/aspectj/DataScopeAspect.java` | AOP 切面，拼接 SQL 条件 |
| `BaseEntity` | `youkang-common/core/domain/BaseEntity.java` | 提供 `params` Map 存储注入的 SQL |
| `SysRole.dataScope` | 数据库字段 | 角色的数据权限级别（1-5） |
| `sys_role_dept` | 数据库表 | 角色与部门的自定义关联（仅级别2使用） |
| `sys_dept.ancestors` | 数据库字段 | 部门祖级链，用于"本部门及以下"查询 |
...
| 级别 | 代码 | 名称 | 数据范围 |
|------|------|------|---------|
| 全部数据 | `1` | DATA_SCOPE_ALL | 不添加任何过滤条件 |
| 自定义 | `2` | DATA_SCOPE_CUSTOM | 根据角色关联的部门进行过滤 |
| 本部门 | `3` | DATA_SCOPE_DEPT | 仅用户所在部门的数据 |
| 本部门及以下 | `4` | DATA_SCOPE_DEPT_AND_CHILD | 用户所在部门及所有子部门的数据 |
| 仅本人 | `5` | DATA_SCOPE_SELF | 仅当前用户创建的数据 |
...
public @interface DataScope {
    /** 部门表的别名（对应 SQL 中的 LEFT JOIN sys_dept d） */
    String deptAlias() default "";

    /** 用户表的别名（对应 SQL 中的 sys_user u），仅级别5需要 */
    String userAlias() default "";

    /** 权限字符，用于多角色匹配，默认取 @PreAuthorize 的值 */
...
- **`deptAlias`**：必填。SQL 中部门表的别名，生成的条件会用 `d.dept_id = xxx` 这样的形式
- **`userAlias`**：仅"仅本人"(5)时需要。如果填了，生成 `u.user_id = xxx`；如果没填，则生成 `d.dept_id = 0`（查不到任何数据）
- **`permission`**：一般留空。默认取 Controller 上的权限注解值，用于多角色场景下筛选匹配的角色

### 4.5 底层实现详解

#### 4.5.1 BaseEntity —— SQL 条件的载体

```java
// youkang-common/core/domain/BaseEntity.java
public class BaseEntity {
    /** 请求参数（含数据权限注入的 SQL 片段） */
...
所有需要数据权限的实体类都继承 `BaseEntity`。AOP 切面将拼接好的 SQL 条件写入 `params` 这个 Map 的 `"dataScope"` key 中。

#### 4.5.2 DataScopeAspect —— 核心切面逻辑

**文件位置**：`youkang-framework/src/main/java/com/youkang/framework/aspectj/DataScopeAspect.java`

完整的执行流程：

```
方法调用被 @DataScope 标注的 Service 方法
  ↓
① clearDataScope() —— 清空 params.dataScope，防止 SQL 注入
  ↓
② SecurityUtils.getLoginUser() —— 获取当前登录用户
  ↓
③ 判断是否超级管理员 → 是 → 跳过，不过滤
  ↓ 否
④ 遍历用户所有角色，逐个生成 SQL 条件
  ↓
⑤ 将拼接好的 SQL 写入 BaseEntity.params["dataScope"]
  ↓
⑥ MyBatis 执行时 ${params.dataScope} 被替换为实际条件
```
...
遍历用户所有角色时，按以下规则生成 SQL（`dataScope` 为角色的 `data_scope` 字段值）：
...
for (SysRole role : user.getRoles()) {
    ...
    // 跳过已处理过的级别 和 已停用的角色
    if (conditions.contains(dataScope) || 角色已停用) continue;
    // 跳过不包含当前权限字符的角色
    if (!role.getPermissions().contains(permission)) continue;

    switch (dataScope) {
        case "1": // 全部数据
            sqlString = new StringBuilder(); // 清空所有条件
            conditions.add(dataScope);
            break; // 直接跳出，不过滤

        case "2": // 自定义
            sqlString.append(" OR {deptAlias}.dept_id IN " +
        ...
        case "4": // 本部门及以下
            sqlString.append(" OR {deptAlias}.dept_id IN " +
                "(SELECT dept_id FROM sys_dept WHERE dept_id = {user.deptId} " +
                "OR find_in_set({user.deptId}, ancestors))");
            break;
        ...
        case "5": // 仅本人
            if (StringUtils.isNotBlank(userAlias)) {
                ...
                // 没有 userAlias → dept_id = 0，查不到任何数据
...
// 如果所有角色都不匹配当前权限字符，也设为 dept_id = 0
if (conditions.isEmpty()) {
    sqlString.append(" OR {deptAlias}.dept_id = 0");
}

// 最终写入 BaseEntity
// 注意 substring(4) 是为了去掉开头的 " OR "
baseEntity.getParams().put("dataScope", " AND (" + sqlString.substring(4) + ")");
```

**重要细节**：

- 遇到级别 1（全部数据）时直接 `break` 并清空所有条件，意味着只要有一个角色是"全部数据"就能看到所有数据
- 多个角色的条件用 `OR` 连接，取并集（权限叠加）
- 已停用的角色会被跳过
- 如果角色的 `permissions` 不包含当前接口的权限字符，该角色也会被跳过
- 如果没有任何角色匹配，默认 `dept_id = 0`（安全兜底，查不到数据）

#### 4.5.4 sys_dept.ancestors 字段

`ancestors` 字段存储部门从根到当前节点的完整路径，用逗号分隔：

```
部门树结构：
有康科技(100)
├── 深圳总部(200)
│   ├── 研发部(201)      → ancestors = "0,100,200"
│   └── 市场部(202)      → ancestors = "0,100,200"
└── 上海分公司(300)
    └── 销售部(301)      → ancestors = "0,100,300"
```

"本部门及以下"(4) 的查询利用 `find_in_set` 实现：

```sql
-- 用户部门为 200（深圳总部），查找本部门及子部门
SELECT dept_id FROM sys_dept
WHERE dept_id = 200                    -- 本部门
   OR find_in_set(200, ancestors)      -- ancestors 中包含 200 的都是子部门
-- 结果: 200, 201, 202
```

### 4.6 SQL 条件生成示例

假设用户 `userId=5, userName="zhangsan", deptId=201`，角色为"研发部主管"：
...
#### 级别 2：自定义数据权限（假设 sys_role_dept 中 role_id=2 关联了 dept_id 200, 201）
...
AND (d.dept_id IN (
    SELECT dept_id FROM sys_dept
    WHERE dept_id = 201 OR find_in_set(201, ancestors)
))
```
...
- `deptAlias = "d"` → SQL 中 `sys_dept` 的别名是 `d`
- `userAlias = "u"` → SQL 中 `sys_user` 的别名是 `u`（仅本人权限时用到）

#### 第二步：Mapper XML 中放占位符
...
<select id="selectUserList" parameterType="SysUser" resultMap="SysUserResult">
    ...
    <!-- 数据权限过滤（由 DataScopeAspect 自动注入） -->
    ${params.dataScope}
</select>
```

**关键点**：
- `${params.dataScope}` 必须用 `${}` 而不是 `#{}`，因为注入的是 SQL 片段不是参数值
- SQL 中必须有 `left join sys_dept`，且别名与 `deptAlias` 一致
- 需要级别 5 时，SQL 中必须有用户表别名与 `userAlias` 一致
- **安全防护**：切面在每次执行前会先 `clearDataScope()` 清空，防止恶意注入

#### 第三步：Controller 层加权限注解（可选但推荐）

```java
@PreAuthorize("@ss.hasPermi('system:user:list')")
@GetMapping("/list")
public YKResponse<Object> list(SysUser user) {
...
| Service 方法 | deptAlias | userAlias | 对应 Mapper |
|-------------|-----------|-----------|-------------|
| `SysUserServiceImpl.selectUserList()` | `"d"` | `"u"` | `SysUserMapper.selectUserList` |
| `SysUserServiceImpl.selectAllocatedList()` | `"d"` | `"u"` | `SysUserMapper.selectAllocatedList` |
| `SysUserServiceImpl.selectUnallocatedList()` | `"d"` | `"u"` | `SysUserMapper.selectUnallocatedList` |
| `SysDeptServiceImpl.selectDeptList()` | `"d"` | — | `SysDeptMapper.selectDeptList` |
| `SysRoleServiceImpl.selectRoleList()` | `"d"` | — | `SysRoleMapper.selectRoleList` |
...
2. 跳过已停用的角色（status = '1'）
3. 跳过不包含当前权限字符的角色
4. 每个角色的条件用 OR 拼接（取并集）
5. 遇到"全部数据"(1) 时，清空所有条件并停止（最大权限胜出）
6. 去重：已处理过的 dataScope 级别不再重复处理
```

**示例**：用户同时拥有"本部门"角色和"自定义"角色

```
角色A: dataScope=3（本部门），deptId=201
角色B: dataScope=2（自定义），关联部门 [200, 300]

生成的SQL:
AND (d.dept_id = 201                               -- 角色A
  OR d.dept_id IN (SELECT dept_id FROM sys_role_dept WHERE role_id = 2))  -- 角色B
```

用户能看到**本部门数据**与**自定义部门数据**的并集。

### 4.10 超级管理员豁免

```java
// DataScopeAspect.java 第74行
...
`isAdmin()` 判断的是 `userId == 1`，超级管理员的查询不会被追加任何数据权限条件。
...
| ancestors | varchar(255) | 祖级列表，逗号分隔（如 `0,100,200`） |
...
**`ancestors` 字段至关重要**：它是"本部门及以下"查询的基础。新增部门时必须正确维护此字段，格式为从根到父的完整路径，如 `0,100,200`。

#### sys_role_dept（角色-部门关联表）

```sql
-- 仅当角色的 data_scope = 2（自定义）时使用
...
1. 在角色管理界面编辑角色
2. 选择数据权限范围（1-5）
3. 如果选择"自定义数据权限"(2)，勾选允许查看的部门
4. 保存后，系统自动维护 `sys_role_dept` 关联表
5. 该角色下的用户下次查询时自动应用新的数据权限

### 4.13 为新模块添加数据权限

假设要给"订单管理"添加数据权限：

**1) 实体类继承 BaseEntity**（或已有 `dept_id` + `create_by` 字段）

```java
public class OrderInfo extends BaseEntity {
    private Long orderId;
    private Long deptId;      // 必须有部门ID字段
    private String createBy;  // 仅本人权限时需要
    // ...
}
```

**2) Service 层加 @DataScope**

```java
@Override
@DataScope(deptAlias = "d")
public List<OrderInfo> selectOrderList(OrderInfo query) {
...
**4) 确保查询参数是 BaseEntity 的子类**

方法第一个参数必须是 `BaseEntity` 的子类（切面通过 `joinPoint.getArgs()[0]` 获取参数对象）。

### 4.14 注意事项与常见陷阱

1. **`${params.dataScope}` 不是 `#{params.dataScope}`**：必须用 `${}` 因为注入的是 SQL 片段
2. **表别名必须一致**：`deptAlias="d"` 要求 SQL 中 `LEFT JOIN sys_dept d`
3. **第一个参数必须是 BaseEntity 子类**：切面只处理 `joinPoint.getArgs()[0]`
4. **ancestors 字段必须正确**：部门增删改时要同步维护 ancestors 路径
5. **超级管理员 userId=1 不会被过滤**：设计测试数据时注意
6. **仅本人(5)需要 userAlias**：不填则安全兜底为 `dept_id = 0`，查不到数据
7. **权限修改后需重新登录**：权限缓存在 Redis 的 LoginUser 中，旧令牌仍用旧权限
8. **SQL 注入安全**：切面每次执行前会 `clearDataScope()` 清空，SQL 片段由后端拼接不依赖用户输入
