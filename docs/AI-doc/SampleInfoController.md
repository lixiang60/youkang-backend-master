...
### 19. 加测
- **路径**: `/addTest` (完整: `/order/sample/addTest`)
- **方法**: `POSTMAPPING`
- **说明**: 根据原样品信息创建新的加测样品记录，流程设置为反应生产，返回状态设置为模板成功
- **权限**: `order:sample:addTest`
- **请求参数**: `[SampleAddTestReq](#sampleaddtestreq)`
- **响应结构**: `R<Long>` (返回新生成的生产编号)

**业务规则**:
1. 根据生产编号查询原样品信息
2. 生成新的订单号（格式：yyyyMMddHHmmss + 3位毫秒）
3. 生成新的生产编号（格式：yyMMdd + 4位序号）
4. 创建新样品记录：
   - 订单号：新生成
   - 历史订单号：接口上传的订单编号
   - 生产编号：新生成
   - 样品对应号：接口上传的订单编号（纯数字时转换，否则使用原样品的样品对应号）
   - 引物名称、引物序列、引物浓度、备注：按接口上传字段填充
   - 流程名称：反应生产
   - 返回状态：模板成功
   - 其余字段：复制原样品信息
5. 清除模板和反应的板号孔号信息
...
| 字段 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| produceId | `Long` | 是 | 原生产编号（用于查询原样品信息） |
| orderId | `String` | 是 | 订单编号（将成为历史订单号和样品对应号） |
| primer | `String` | 否 | 引物名称 |
| seq | `String` | 否 | 引物序列 |
